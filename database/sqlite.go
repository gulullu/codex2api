...
func (db *DB) migrateSQLite(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS system_settings (
				...
				public_image_studio_page_enabled INTEGER DEFAULT 0,
				...
				scheduler_mode TEXT DEFAULT 'round_robin',
					...
					codex_ws_idle_reclaim_enabled INTEGER DEFAULT 0,
					codex_ws_idle_reclaim_percent INTEGER DEFAULT 0,
					codex_ws_idle_reclaim_idle_sec INTEGER DEFAULT 600,
	...
	columns := []struct {
		...
		{"usage_logs", "channel", "TEXT DEFAULT 'codex'"},
		...
		{"system_settings", "codex_ws_idle_reclaim_enabled", "INTEGER DEFAULT 0"},
		{"system_settings", "codex_ws_idle_reclaim_percent", "INTEGER DEFAULT 0"},
		{"system_settings", "codex_ws_idle_reclaim_idle_sec", "INTEGER DEFAULT 600"},
		...
		{"system_settings", "public_image_studio_page_enabled", "INTEGER DEFAULT 0"},
	...
	indexStatements := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_usage_logs_cyber_scope_created_at ON usage_logs(upstream_error_kind, upstream_account_type, created_at DESC, id DESC);`,
...
func (db *DB) getChartAggregationSQLite(ctx context.Context, start, end time.Time, bucketMinutes int, channel string) (*ChartAggregation, error) {
	...
	if channel != "" {
		query += " AND " + usageLogChannelSQLPredicate("channel", "$3")
...
func (db *DB) getUsageStatsSQLite(ctx context.Context, rangeStart, rangeEnd time.Time, channel string) (*UsageStats, error) {
	...
	if channel != "" {
		...
		query += " AND " + usageLogChannelSQLPredicate("channel", placeholder)
		args = append(args, channel)
	}
