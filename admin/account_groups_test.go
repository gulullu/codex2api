...
func TestAccountGroupBaseConcurrencyOverrideAPIThreeState(t *testing.T) {
	...
	// 50 和本地旧补丁的 1000 都不再是上限。
	recorder = patch(`{"base_concurrency_override":1001}`)
	if recorder.Code != http.StatusOK {
		t.Fatalf("patch 1001 status = %d, want %d: %s", recorder.Code, http.StatusOK, recorder.Body.String())
	}
	if got := account.GetBaseConcurrencyEffective(); got != 1001 {
		t.Fatalf("runtime effective after high-value patch = %d, want 1001", got)
	}
	// 恢复到 2，后续断言依赖该值
	recorder = patch(`{"base_concurrency_override":2}`)
	if recorder.Code != http.StatusOK {
...
func TestCreateAccountGroupRejectsInvalidBaseConcurrencyOverride(t *testing.T) {
	...
	// 无上限：超过本地旧补丁的 1000 也应允许。
	recorder := invokeAccountGroupHandler(t, http.MethodPost, "/api/admin/account-groups", nil,
		`{"name":"large","base_concurrency_override":1001}`, handler.CreateAccountGroup)
	if recorder.Code != http.StatusOK {
		t.Fatalf("create with 1001 status = %d, want %d: %s", recorder.Code, http.StatusOK, recorder.Body.String())
	}
}
