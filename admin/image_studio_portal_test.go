...
func TestPortalImageJobIsolationAndAuth(t *testing.T) {
	...
	// The public portal must fail closed until an administrator explicitly enables it.
	ctx := context.Background()
	enabled, err := handler.PublicImageStudioPageEnabled(ctx)
	if err != nil {
		t.Fatalf("PublicImageStudioPageEnabled(default): %v", err)
	}
	if enabled {
		t.Fatal("PublicImageStudioPageEnabled(default) = true, want false")
	}

	// Explicitly enable the portal and register routes.
