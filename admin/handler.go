...
type settingsResponse struct {
	...
	CodexWSIdleReclaimEnabled                  bool                           `json:"codex_ws_idle_reclaim_enabled"`
	CodexWSIdleReclaimPercent                  int                            `json:"codex_ws_idle_reclaim_percent"`
	CodexWSIdleReclaimIdleSec                  int                            `json:"codex_ws_idle_reclaim_idle_sec"`
...
func (h *Handler) GetSettings(c *gin.Context) {
	...
	c.JSON(http.StatusOK, settingsResponse{
		...
		CodexWSIdleReclaimEnabled:                  h.store.CodexWSIdleReclaimEnabled(),
		CodexWSIdleReclaimPercent:                  h.store.CodexWSIdleReclaimPercent(),
		CodexWSIdleReclaimIdleSec:                  h.store.CodexWSIdleReclaimIdleSec(),
...
func (h *Handler) UpdateSettings(c *gin.Context) {
	...
	if req.CodexWSIdleReclaimEnabled != nil {
		h.store.SetCodexWSIdleReclaimEnabled(*req.CodexWSIdleReclaimEnabled)
		runtimeCfg.CodexWSIdleReclaimEnabled = *req.CodexWSIdleReclaimEnabled
		log.Printf("设置已更新: codex_ws_idle_reclaim_enabled = %t", *req.CodexWSIdleReclaimEnabled)
	}

	if req.CodexWSIdleReclaimPercent != nil {
		v := database.NormalizeCodexWSIdleReclaimPercent(*req.CodexWSIdleReclaimPercent)
		h.store.SetCodexWSIdleReclaimPercent(v)
		runtimeCfg.CodexWSIdleReclaimPercent = v
		log.Printf("设置已更新: codex_ws_idle_reclaim_percent = %d", v)
	}

	if req.CodexWSIdleReclaimIdleSec != nil {
		v := database.NormalizeCodexWSIdleReclaimIdleSec(*req.CodexWSIdleReclaimIdleSec)
		h.store.SetCodexWSIdleReclaimIdleSec(v)
		runtimeCfg.CodexWSIdleReclaimIdleSec = v
		log.Printf("设置已更新: codex_ws_idle_reclaim_idle_sec = %d", v)
	}
