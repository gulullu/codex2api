...
func TestBatchUpdateAccountsAllowsResponsesAPIConcurrencyTo10000(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db := newTestAdminDB(t)
	firstID, err := db.InsertOpenAIResponsesAccount(context.Background(), "relay-one", map[string]interface{}{
		"upstream_type": "openai_responses",
		"api_key":       "test-key-one",
		"base_url":      "https://relay-one.example.com",
	}, "")
	if err != nil {
		t.Fatalf("InsertOpenAIResponsesAccount(first): %v", err)
	}
	secondID, err := db.InsertOpenAIResponsesAccount(context.Background(), "relay-two", map[string]interface{}{
		"upstream_type": "openai_responses",
		"api_key":       "test-key-two",
		"base_url":      "https://relay-two.example.com",
	}, "")
	if err != nil {
	...
	recorder := httptest.NewRecorder()
	ginCtx, _ := gin.CreateTestContext(recorder)
	body := fmt.Sprintf(`{"ids":[%d,%d],"base_concurrency_override":10000}`, firstID, secondID)
	ginCtx.Request = httptest.NewRequest(http.MethodPost, "/api/admin/accounts/batch-update", strings.NewReader(body))
	ginCtx.Request.Header.Set("Content-Type", "application/json")
	handler.BatchUpdateAccounts(ginCtx)
	if recorder.Code != http.StatusOK {
	...
	for _, accountID := range []int64{firstID, secondID} {
		...
		if !row.BaseConcurrencyOverride.Valid || row.BaseConcurrencyOverride.Int64 != 10000 {
			t.Fatalf("account %d base concurrency = %+v, want 10000", accountID, row.BaseConcurrencyOverride)
		}
	}
}
