...
import type { AccountGroup, PromptFilterLog, PromptFilterMatch, PromptFilterRule, PromptFilterRulesResponse, PromptFilterVerdict, PromptIntelligenceCandidate, PromptIntelligenceRun, SemanticReviewConnectionTestResponse, SemanticReviewProvider, SemanticReviewStrategy, SystemSettings } from '../types'
...
function parseAdvancedProtection(raw: string): AdvancedProtectionConfig {
  try {
    ...
    return {
      ...
      output: { ...defaultAdvancedProtection.output, ...(value.output || {}), enabled: false },
        ...
        <AdvancedPanel title={t('promptFilter.outputScanTitle')}>
          <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/20 p-3">
            ...
            <p className="text-xs leading-relaxed text-muted-foreground">
              {t('promptFilter.outputManagedBySub2')}
            </p>
          </div>
        </AdvancedPanel>
      </div>
...
function DocsView() {
  ...
  const sections = useMemo<DocsSection[]>(() => [
    ...
    },
    {
      ...
      cards: [
        { title: t('promptFilter.modeMonitor'), body: t('promptFilter.docs.modes.monitor'), tone: 'success' },
      ],
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(190px,1fr))] gap-4">
              ...
              <Field label={t('promptFilter.threshold')}>
                <DraftNumberInput min={1} max={500} value={form.prompt_filter_threshold} onValueChange={(value) => setForm((current) => ({ ...current, prompt_filter_threshold: value }))} />
              </Field>
              <Field label={t('promptFilter.strictThreshold')}>
                <DraftNumberInput min={1} max={1000} value={form.prompt_filter_strict_threshold} onValueChange={(value) => setForm((current) => ({ ...current, prompt_filter_strict_threshold: value }))} />
              </Field>
