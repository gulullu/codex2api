...
import {
  ...
  resolveBatchBaseConcurrencyMax,
} from "./accountBatchUpdate.ts";

test("resolveBatchBaseConcurrencyMax allows 10000 only for fully known Responses accounts", () => {
  const accounts = [
    { id: 1, openai_responses_api: true },
    { id: 2, openai_responses_api: true },
    { id: 3, openai_responses_api: false },
  ];

  assert.equal(resolveBatchBaseConcurrencyMax([1, 2], accounts), 10000);
  assert.equal(resolveBatchBaseConcurrencyMax([1, 3], accounts), 50);
  assert.equal(resolveBatchBaseConcurrencyMax([1, 99], accounts), 50);
  assert.equal(resolveBatchBaseConcurrencyMax([], accounts), 50);
});
