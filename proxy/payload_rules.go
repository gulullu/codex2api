...
type payloadRulesPreAppliedKey struct{}
type payloadRuleSnapshotContextKey struct{}

const payloadRuleSnapshotGinKey = "codex2api_payload_rule_snapshot"
const payloadRuleIdentityGinKey = "codex2api_payload_rule_identity"

func clonePayloadRuleIdentity(id *PayloadRuleIdentity) *PayloadRuleIdentity {
	if id == nil {
		return nil
	}
	frozen := *id
...
// withPayloadRulesPreApplied freezes the payload-rule snapshot used by the
// pre-routing safety scan. ExecuteRequest must not apply the rules a second
// time, otherwise append actions duplicate and a hot configuration update can
// introduce unscanned instructions between routing and the upstream request.
func withPayloadRulesPreApplied(ctx context.Context) context.Context {
	if ctx == nil {
		return ctx
	}
	return context.WithValue(ctx, payloadRulesPreAppliedKey{}, true)
}

func payloadRulesPreAppliedFromContext(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	applied, _ := ctx.Value(payloadRulesPreAppliedKey{}).(bool)
	return applied
}

// withPayloadRuleSnapshot binds the immutable rule-set pointer captured when
// the request entered the handler to the detached upstream context. Payload
// rule hot reloads publish a new pointer, so the captured set remains stable
// for every rebuild/retry performed by this logical request.
func withPayloadRuleSnapshot(ctx context.Context, snapshot *PayloadRuleSet) context.Context {
	if ctx == nil {
		return ctx
	}
	return context.WithValue(ctx, payloadRuleSnapshotContextKey{}, normalizePayloadRuleSnapshot(snapshot))
}

func payloadRuleSnapshotFromContext(ctx context.Context) (*PayloadRuleSet, bool) {
	if ctx == nil {
		return nil, false
	}
	snapshot, ok := ctx.Value(payloadRuleSnapshotContextKey{}).(*PayloadRuleSet)
	if !ok {
		return nil, false
	}
	return normalizePayloadRuleSnapshot(snapshot), true
}
...
var emptyPayloadRuleSet = &PayloadRuleSet{}

func normalizePayloadRuleSnapshot(rs *PayloadRuleSet) *PayloadRuleSet {
	if rs == nil {
		return emptyPayloadRuleSet
	}
	return rs
}
...
// beginPayloadRuleRequest assigns the currently published snapshot to a new
// logical request. HTTP handlers call it once on entry. A multi-turn downstream
// WebSocket calls it once for every response.create turn because one gin
// context outlives multiple logical requests.
func beginPayloadRuleRequest(c *gin.Context) *PayloadRuleSet {
	snapshot := normalizePayloadRuleSnapshot(CurrentPayloadRules())
	if c != nil {
		c.Set(payloadRuleSnapshotGinKey, snapshot)
	}
	return snapshot
}

// beginPayloadRuleRequest freezes both configuration inputs that participate
// in payload-rule matching. A downstream WebSocket calls this once per
// response.create turn, so a later turn sees a newly published rule set and
// current API-key/group metadata while the active turn remains immutable.
func (h *Handler) beginPayloadRuleRequest(c *gin.Context) *PayloadRuleSet {
	snapshot := beginPayloadRuleRequest(c)
	if c != nil {
		c.Set(payloadRuleIdentityGinKey, clonePayloadRuleIdentity(h.payloadRuleIdentity(c)))
	}
	return snapshot
}

// freezePayloadRuleSnapshot returns the exact immutable rule set assigned to
// this request. The first call stores the currently published pointer on the
// gin context; later translations, encrypted-content rebuilds, WS rebuilds,
// retries, and service-tier accounting all see the same snapshot.
func freezePayloadRuleSnapshot(c *gin.Context) *PayloadRuleSet {
	if c == nil {
		return normalizePayloadRuleSnapshot(CurrentPayloadRules())
	}
	if value, exists := c.Get(payloadRuleSnapshotGinKey); exists {
		if snapshot, ok := value.(*PayloadRuleSet); ok {
			return normalizePayloadRuleSnapshot(snapshot)
		}
	}
	snapshot := normalizePayloadRuleSnapshot(CurrentPayloadRules())
	c.Set(payloadRuleSnapshotGinKey, snapshot)
...
// freezePayloadRuleIdentity returns the deep-copied identity assigned at the
// logical request boundary. The compatibility fallback captures on first use
// for internal/unit-test callers that invoke prepareCodexPayloadRules directly.
func (h *Handler) freezePayloadRuleIdentity(c *gin.Context) *PayloadRuleIdentity {
	if c == nil {
		return clonePayloadRuleIdentity(h.payloadRuleIdentity(c))
	}
	if value, exists := c.Get(payloadRuleIdentityGinKey); exists {
		identity, _ := value.(*PayloadRuleIdentity)
		return identity
	}
	identity := clonePayloadRuleIdentity(h.payloadRuleIdentity(c))
	c.Set(payloadRuleIdentityGinKey, identity)
	return identity
}
...
func ApplyPayloadRulesToBody(body []byte, model string, headers http.Header, identity *PayloadRuleIdentity) []byte {
	return ApplyPayloadRuleSetToBody(CurrentPayloadRules(), body, model, headers, identity)
}

// ApplyPayloadRuleSetToBody applies one explicit immutable snapshot. Request
// paths must use this form after freezePayloadRuleSnapshot so a hot reload
// cannot alter an in-flight request between routing scan and upstream write.
func ApplyPayloadRuleSetToBody(rs *PayloadRuleSet, body []byte, model string, headers http.Header, identity *PayloadRuleIdentity) []byte {
	rs = normalizePayloadRuleSnapshot(rs)
...
// applyPayloadRulesForExecute is the single ExecuteRequest-side gate. Most
// callers keep the historical late application path. Text handlers that must
// inspect the exact OAuth payload before account selection pre-apply the
// rules, pass that body to ExecuteRequest, and mark the upstream context with
// withPayloadRulesPreApplied.
func applyPayloadRulesForExecute(ctx context.Context, body []byte, model string, headers http.Header) []byte {
	if payloadRulesPreAppliedFromContext(ctx) {
		return body
	}
	snapshot, ok := payloadRuleSnapshotFromContext(ctx)
	if !ok {
		// Compatibility for non-handler/internal callers. Production request
		// handlers bind a frozen snapshot before creating the upstream request.
		snapshot = CurrentPayloadRules()
	}
	return ApplyPayloadRuleSetToBody(snapshot, body, model, headers, PayloadRuleIdentityFromContext(ctx))
}

// prepareCodexPayloadRules builds the exact body a Codex OAuth account would
// receive. It deliberately does not mutate the base body: Relay accounts use
// that untouched body because payload rules are an OAuth-Codex feature.
//
// The bool reports that the hypothetical OAuth body was prepared (even if no
// action matched). It is intentionally independent of the accounts currently
// visible to the scheduler: an OAuth account can appear/become reachable after
// the routing scan, and any injected instructions still must have been scanned.
// Relay selection continues to send the untouched base body.
func (h *Handler) prepareCodexPayloadRules(c *gin.Context, body []byte, model string, _ auth.AccountFilter) ([]byte, bool) {
...
// scanned and frozen before account selection. Payload rules cannot rewrite
// input, therefore both candidates must expose the same repairable history.
// Crucially, this function never re-runs rule matching on the repaired shape:
// a compaction item converted to a developer message cannot make a previously
// unmatched rule fire midway through a logical request, and append actions
// cannot be duplicated.
func repairFrozenPayloadRuleBodies(baseBody []byte, oauthBody []byte) ([]byte, []byte, encryptedContentRepair, bool) {
	repairedBase, baseRepair := repairInvalidEncryptedContentFromResponsesBody(baseBody)
	if !baseRepair.Changed || baseRepair.InputEmpty {
		return baseBody, oauthBody, baseRepair, false
	}
	repairedOAuth, oauthRepair := repairInvalidEncryptedContentFromResponsesBody(oauthBody)
	if !oauthRepair.Changed || oauthRepair.InputEmpty ||
		oauthRepair.Dropped != baseRepair.Dropped ||
		oauthRepair.Converted != baseRepair.Converted {
		return baseBody, oauthBody, baseRepair, false
	}
	return repairedBase, repairedOAuth, baseRepair, true
}

func preparedResponsesInputRaw(body []byte) string {
...
func EffectiveRequestedServiceTier(body []byte, model string, headers http.Header, identity *PayloadRuleIdentity) string {
	return EffectiveRequestedServiceTierWithSnapshot(CurrentPayloadRules(), body, model, headers, identity)
}

// EffectiveRequestedServiceTierWithSnapshot attributes requested/billing tier
// using the same frozen rules that produced and scanned the OAuth body.
func EffectiveRequestedServiceTierWithSnapshot(snapshot *PayloadRuleSet, body []byte, model string, headers http.Header, identity *PayloadRuleIdentity) string {
	if responsesBodyRequestsImageGeneration(body) {
		return extractServiceTier(body)
	}
	return extractServiceTier(ApplyPayloadRuleSetToBody(snapshot, body, model, headers, identity))
}
