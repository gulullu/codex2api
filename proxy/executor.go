...
//
// 注意：账号调度键由 resolveSchedulerAffinityID 单独解析，只接受专用 affinity
// header 或显式 session 标识；本函数只接收 upstreamSeed，因此内容/API Key
// fallback 仅用于上游缓存身份，不会把独立请求粘到同一本地账号。
...
func ExecuteRequest(ctx context.Context, account *auth.Account, requestBody []byte, sessionID string, proxyOverride string, apiKey string, deviceCfg *DeviceProfileConfig, headers http.Header, useWebsocket ...bool) (*http.Response, error) {
	...
	if !responsesBodyRequestsImageGeneration(requestBody) {
		...
		requestBody = applyPayloadRulesForExecute(ctx, requestBody, gjson.GetBytes(requestBody, "model").String(), headers)
	...
	if wantWebsocket && WebsocketExecuteFunc != nil {
		...
		observeUpstreamTransport(ctx, true, "", 0, 0)
	...
	if !wantWebsocket {
		observeDefaultUpstreamTransport(ctx, false)
	}
...
// ResolveSessionID 从下游请求提取或生成通用的上游/cache session ID。
// 主请求 handler 的账号调度不得直接使用本函数，而应调用
// resolveSchedulerAffinityID；后者只接受专用 affinity header 或显式 session ID。
// 优先级：
...
//
// 第 6、7 级只为上游 prompt cache/session identity 提供稳定种子，不参与主
// handler 的本地账号调度；因此无显式会话的独立逻辑请求不会因相同内容或共享
// Bearer Key 被固定到同一账号。
...
// resolveSchedulerAffinityID preserves the deliberately narrow rb12 account
// routing key while accepting v2.5.4's dedicated local-only affinity header.
// Content-derived and Bearer-key fallback seeds remain available to upstream
// session/cache identity, but must not pin scheduler selection.
func resolveSchedulerAffinityID(headers http.Header, body []byte) string {
	if localAffinityID := resolveDownstreamAffinityID(headers); localAffinityID != "" {
		return localAffinityID
	}
	return ResolveExplicitSessionID(headers, body)
}
