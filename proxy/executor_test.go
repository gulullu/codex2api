...
func TestResolveSchedulerAffinityIDDoesNotUseContentOrAPIKeyFallback(t *testing.T) {
	body := []byte(`{"model":"gpt-5.4","input":[{"role":"user","content":"stable prompt"}]}`)
	headers := http.Header{"Authorization": []string{"Bearer shared-key"}}

	if got := resolveSchedulerAffinityID(headers, body); got != "" {
		t.Fatalf("scheduler affinity = %q, want empty without an explicit identity", got)
	}

	headers.Set("Session_id", "explicit-session")
	if got := resolveSchedulerAffinityID(headers, body); got != "explicit-session" {
		t.Fatalf("explicit scheduler affinity = %q", got)
	}

	headers.Set("X-Codex2API-Affinity-Key", "tenant-conversation")
	if got, want := resolveSchedulerAffinityID(headers, body), resolveDownstreamAffinityID(headers); got == "" || got != want {
		t.Fatalf("local scheduler affinity = %q, want hashed header %q", got, want)
	}
}
