...
func TestInternalResponseAttributionPreservesParentAuditIdentity(t *testing.T) {
	...
	if !child.GetBool(skipCYBLearningPipelineContextKey) {
		t.Fatal("attributed summary request must skip the custom CYB/Relay pipeline")
	}
...
func TestApplyInternalResponseAttributionNilRemainsAnonymous(t *testing.T) {
	...
	if c.GetBool(skipCYBLearningPipelineContextKey) {
		t.Fatal("nil attribution unexpectedly skipped the custom CYB/Relay pipeline")
	}
}
