...
const (
	skipCYBLearningPipelineContextKey = "skipCYBLearningPipeline"
...
func applyInternalResponseAttribution(c *gin.Context, req *http.Request, attribution *internalResponseAttribution) {
	...
	// Attributed internal calls may inherit the parent's no-affinity routing,
	// but synthetic summary text must not enter Relay audit, CYB learning, pin,
	// replay, or output-filter pipelines as if it were a new client request.
